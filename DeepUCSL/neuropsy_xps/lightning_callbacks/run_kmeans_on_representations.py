...
class KMeansOnRep(pl.callbacks.Callback):
    ...
    def on_test_end(self, trainer, pl_module):
        ...
        val_loader = pl_module.data_manager.get_dataloader(pl_module.fold_index, shuffle=False, test=True).test

        # train representations
        head_vectors = [pl_module.forward(batch.cuda())[0].cpu().detach().numpy() for _, batch, _, _ in train_loader]
        head_vectors = np.array([item for sublist in head_vectors for item in sublist])

        # validation representations
        head_vectors_val = [pl_module.forward(batch.cuda())[0].cpu().detach().numpy() for _, batch, _, _ in val_loader]
        head_vectors_val = np.array([item for sublist in head_vectors_val for item in sublist])

        y_train = np.array([create_diagnosis_and_subtype_dict(labels)["subtype"].detach().numpy() for _, _, labels, _ in train_loader])
        y_train = np.array([item for sublist in y_train for item in sublist])

        y_val = np.array([create_diagnosis_and_subtype_dict(labels)["subtype"].detach().numpy() for _, _, labels, _ in val_loader])
        ...
        pl_module.logger.experiment.add_scalar("Clustering Balanced Accuracy/Test",
                                          balanced_accuracy_clusters,
                                          pl_module.current_epoch)