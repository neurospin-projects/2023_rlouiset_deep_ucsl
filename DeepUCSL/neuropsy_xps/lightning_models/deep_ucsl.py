...
class LitDeepUCSL(LightningModule):
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        conditional_classif_loss, pos_clustering_loss, neg_clustering_loss = self.compute_ucsl_loss(
        ...
        loss = (self.loss_params["conditional_classification_weight"] * conditional_classif_loss +
               self.loss_params["pos_clustering_weight"] * pos_clustering_loss +
                self.loss_params["neg_clustering_weight"] * neg_clustering_loss)
        ...
        batch_dictionary = {
            ...
            "clustering_loss": self.loss_params["pos_clustering_weight"] * pos_clustering_loss + self.loss_params["neg_clustering_weight"] * neg_clustering_loss,
            "y_pred": y_pred,
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        if self.current_epoch > 0:
            ...
            loss, conditional_classif_loss, pos_clustering_loss, neg_clustering_loss = torch.tensor(0.0),torch.tensor(0.0), torch.tensor(0.0), torch.tensor(0.0)
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        # identifying number of correct predictions in a given batch
        head_vector = self.scaler.transform(head_vector.detach())
        Q_probability = predict_proba_from_barycenters(head_vector.cpu().numpy(), self.barycenters)
        y_pred = (torch.tensor(Q_probability) * conditional_probability.cpu()).sum(1)
    ...
    def validation_epoch_end(self, outputs):
        ...
        print("VAL loss : ", (self.loss_params["conditional_classification_weight"] * avg_cdntl_clsf_loss + avg_cluster_loss).item())
    ...
    def training_epoch_end(self, outputs):
        ...
        print("TRAIN /  loss : ", (self.loss_params["conditional_classification_weight"] * avg_cdntl_clsf_loss + avg_cluster_loss).item())
    ...
    def test_epoch_end(self, outputs):
        ...
        representations = torch.cat([x['representations'] for x in outputs], dim=0).view(-1, 128)
        ...
        print("Q TEST SHAPE IS: ", Q_test.shape)

        # calculating correct and total predictions
        balanced_accuracy = balanced_accuracy_score(labels, (y_preds>0.5).astype(float))
        ...
        overall_accuracy_Q = overall_accuracy_for_clusters_and_classes(labels, (y_preds>0.5).astype(float), y_subtype, np.argmax(Q_subtype_test, 1), self.permutation_indices)
        ...
        overall_accuracy_clusters = overall_accuracy_for_clusters_and_classes(labels, (y_preds>0.5).astype(float), y_subtype, y_subtype_pred, self.permutation_indices)
